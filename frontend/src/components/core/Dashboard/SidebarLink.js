...
const SidebarLink = ({link,iconName}) => {
  ...
  return (
    <NavLink
        to={link.path}
        //onClick={}
        className={ ` py-2 px-4 relative md:px-8 md:py-2 text-sm font-medium transition-all duration-300 ${matchRoute(link.path) ? "bg-yellow-800 text-yellow-50" :"bg-opacity-0 text-opacity-0"}`}
    >
